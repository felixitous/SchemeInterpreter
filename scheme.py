...
class Frame(object):
    ...
    def lookup(self, symbol):
        ...
        elif self.parent is not None: #check self.parent.bindings CHECK!!!
    ...
    def make_call_frame(self, formals, vals):
        ...
        if len(formals) > len(vals):
            ...
            def construct(formals, vals):
                if formals == nil:
                    return frame 
                else:
                    frame.define(formals.first, vals.first)
                    return construct(formals.second, vals.second)
            return construct(formals, vals)
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        ...
        if scheme_symbolp(target.first):
            ...
            raise SchemeError(str(target.first) + ' is not a symbol')
    else:
...
def do_let_form(vals, env):
    ...
    for elem in bindings:
        names = Pair(elem.first, names)
        values = Pair(scheme_eval(elem.second.first, env), values)
...
def do_and_form(vals, env):
    ...
    if vals == nil:
        return True 
    for number in range(0, len(vals) - 1): #short-circuit, if false, immediately returns false
        if not scheme_true(scheme_eval(vals[number], env)):
            return False
    return vals[len(vals) - 1] #returns last value, if false, it would simply eval itself
...
def do_or_form(vals, env):
    ...
    if vals == nil:
        return False
    for number in range(len(vals)-1):  
        if scheme_true(scheme_eval(vals[number], env)):
            return scheme_eval(quote(vals[number]), env)
    ...
    for i, clause in enumerate(vals):
        ...
        if scheme_true(test):
            if clause.second == nil:
                return quote(test)
            return do_begin_form(clause.second, env) #should we use do_begin_form or Pair('begin', Pair)
    return okay
...
def check_formals(formals):
    ...
    hold = []
    for items in formals:
        if not scheme_symbolp(items):
            raise SchemeError("Non-symbol: " + str(items))
        if items in hold:
...
def scheme_optimized_eval(expr, env):
    ...
    while True:
        ...
        if (scheme_symbolp(first) # first might be unhashable
            ...
            new_proc = scheme_optimized_eval(first, env)
            args = rest.map(lambda operand: scheme_optimized_eval(operand, env))
            if isinstance(new_proc, PrimitiveProcedure):
                return apply_primitive(new_proc, args, env)
            elif isinstance(new_proc, LambdaProcedure):
                expr, env = new_proc.body, new_proc.env.make_call_frame(new_proc.formals, args)
            elif isinstance(new_proc, MuProcedure):
                expr, env = new_proc.body, env.make_call_frame(new_proc.formals, args)
            else:
                raise SchemeError("Cannot call {0}".format(str(procedure)))
